...
window.PACKAGES = {
  ...
  "clearview-deck": [
    ...
    // ── Added from a flat file-ID list (name/ID/MIME/URL only — no parent
    // folder). The two sets below are an INFERENCE from the filenames
    // themselves ("...-14-AI.jpg" vs "...-14.jpg", same shot numbers 01–14
    // in both): looks like the same 14 shots rendered twice, once through
    // an AI pass and once without. If that's wrong — e.g. they're actually
    // one set, or the AI ones supersede the others — just merge/rename;
    // nothing else keys off the split.
    {
      slug: "ai-renderings",
      title: "AI Renderings",
      issued: "2026-08-21",
      note: "AI-rendered pass of shots 01–14. Grouping inferred from filenames — confirm against the source.",
      driveFolderId: "",
      items: [
...

    {
      ...
      title: "Renderings",
      issued: "2026-08-21",
      note: "Same shot numbers as AI Renderings, without the AI pass. Grouping inferred from filenames — confirm against the source.",
      driveFolderId: "",
      items: [
    ...
    // These 3 videos carried no project prefix at all in the source list
    // (just "Sequence1.mp4", "Video1.mp4", "Video2.mp4") — filed here only
    // because they sat between Clearview Deck's and La Costa's rows in the
    // list you pasted. That's a weaker signal than the filename-prefix
    // grouping above; move this set under La Costa (or split it) if that's
    // wrong. `type: "video"` opens in the same embedded viewer PDFs use —
    // Drive's file preview plays video natively there — but this is the
    // first video content on the site, so treat playback as unverified
    // until you've actually opened one.
    {
      slug: "videos",
      title: "Videos",
      issued: "",
      note: "Ownership inferred from position in the source list, not a filename match — verify these belong to Clearview Deck.",
      driveFolderId: "",
      items: [
  ...
  /* ══════════════════════════════════════════════════════════════════════
   * la-costa — new project, added entirely from a flat file-ID list (no
   * folder structure was given). Review sets below are inferred from a
   * version tag embedded in each filename (V26, V26.b, V26.c, V28.1,
   * V28.1.B) — the same convention /clearview-deck's own version folders
   * already use (v5, v5-2, v5-2b...), so it's a confident read, but still
   * an inference, not something pulled from real Drive folders. See the
   * matching entry in projects.js for this project's own metadata.
   * ══════════════════════════════════════════════════════════════════════ */
  "la-costa": [
    {
      slug: "v26",
