...
// A version's drawing-set PDF, if one was dropped in assets/ — same
// drop-a-file-and-scan-finds-it treatment as images/videos above. At most
// one PDF per version is expected; if there's more than one, the first in
// natural-sort order wins and the rest are ignored (with a warning). Label
// is a fixed generic string rather than a humanized filename — PDF source
// filenames (full sheet titles/dates) don't make good button copy.
function pickPdf(versionId, assetsDir) {
  const files = fs.readdirSync(assetsDir).filter((f) => PDF_EXT.test(f)).sort(naturalCompare);
  if (!files.length) return null;
  if (files.length > 1) {
    console.warn(
      "[warn] " + versionId + ": multiple PDFs in assets/ (" + files.join(", ") +
      ") — using " + files[0] + ", ignoring the rest"
    );
  }
  return { src: "assets/" + files[0], label: "Drawing Set (PDF)" };
}
...
function renderDataJs(id, meta, thumb, images, videos, hero, pdf) {
  ...
  const pdfJs = pdf
    ? `{ src: ${jsStringLiteral(pdf.src)}, label: ${jsStringLiteral(pdf.label)} }`
    : "null";
 ...
 * files in versions/${id}/assets/ for images and local videos, drop at most
 * one PDF in versions/${id}/assets/ for the drawing-set viewer + download
 * button, or add entries to versions/${id}/videos.json for non-file videos
...
function main() {
  ...
  ids.forEach((id) => {
    ...
    const pdf = pickPdf(id, assetsDir);

    const dataJs = renderDataJs(id, meta, thumb, images, videos, hero, pdf);
    ...
    const pdfNote = pdf ? ", pdf=" + pdf.src : "";
    console.log(
      "[ok] " + id + ": " + images.length + " image(s) (" + metaCount + " with metadata title), " +
      videos.length + " video(s), thumb=" + (thumb || "none") + heroNote + pdfNote
    );
  });
